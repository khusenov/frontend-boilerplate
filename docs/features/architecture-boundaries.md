...
> **`arch:graph` is a working-tree addition, not part of `1c193c6`.** The `arch:graph` script, the
> `dependency-cruiser` devDependency it calls and the generated `docs/architecture-graph.md` are
> the one piece of tooling this document describes that its `Verified against` commit does not
> contain: that commit's `package.json` lists neither the script nor the package, and
> `git log --all -- docs/architecture-graph.md` returns nothing. A clean checkout of the SHA
> therefore answers `npm run arch:graph` with `npm error Missing script: "arch:graph"` and has no
> graph file to open. Every `arch:graph` claim below — its flags, what it rewrites, the shape of
> the output — was verified against the working tree that holds them, and all of it becomes
> reproducible at the commit that lands `package.json`, `package-lock.json` and `docs/` together.
> Everything else in this document is verified against `1c193c6`; see
> [Known limitations](#known-limitations).

| Component                                                                                                                                                    | Layer                           | Responsibility                                                                                                                                                                                           | File                                                                         |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `arch`                                                                                                                                                       | outside layers · root config    | `steiger ./src`: the FSD structure check                                                                                                                                                                 | `package.json`                                                               |
| `defineConfig`                                                                                                                                               | outside layers · root config    | The steiger configuration: `fsd.configs.recommended` plus one `fsd/insignificant-slice` override for two `features` slices                                                                               | `steiger.config.ts`                                                          |
...
| `SESSION_CONSTRUCTOR_NAMES`                                                                                                                                  | outside layers · root config    | The five session constructors, shared by both blocks that ban them                                                                                                                                       | `eslint.config.js`                                                           |
...
| `arch:graph`                                                                                                                                                 | outside layers · root config    | Regenerates the Mermaid dependency graph with dependency-cruiser; working tree only, not in `1c193c6`                                                                                                    | `package.json`                                                               |
...
| `npm run arch:graph`        | dependency-cruiser over `src` (broken down in [Quality gates](./quality-gates.md#npm-scripts)) | Rewrites [`docs/architecture-graph.md`](../architecture-graph.md). Not a gate, and not yet committed — see the note under [Architecture](#architecture) |
...
| `features` | `src/features` | One slice per user action that changes state: its form, its validation schema and the hook that drives it                                                                                                                                            | `entities`, `shared`                                                | `sign-in`, `update-user-name`                                             |
...
| `@/entities/session`           | `entities`   | `createSessionApi`, `createSessionResolver`, `createSessionStarter`, `createSessionStore`, `createSessionTokenSource`, `toSessionObserver`, `SessionResolverProvider`, `useSessionResolver`, `SessionStarterProvider`, `useSessionStarter` | `Credentials`, `SessionObserver`, `SessionResolver`, `SessionStarter`, `SessionStatus`, `SignInOutcome`                                                                                                                                                      | [Session management](./session-management.md); [Authenticated route guard](./route-guard.md) (`createSessionResolver`, `SessionResolver`, `useSessionResolver`, `SessionResolverProvider`) |
...
The constructors in this table — `createHttpClient`, `createQueryClient`, `createI18n`, the five
...
| `fsd/insignificant-slice`          | An `entities`, `features` or `widgets` slice that no other layer imports, or that exactly one other slice imports. A single importer in `app` passes, and `pages` slices are never checked. Off for `./src/features/sign-in/**` and `./src/features/update-user-name/**`                                                                                                                                                             |
...
| `SESSION_CONSTRUCTOR_NAMES` from `@/entities/session`: `createSessionApi`, `createSessionResolver`, `createSessionStarter`, `createSessionStore`, `createSessionTokenSource` | Construction ban         | The same                                                                                         | None                                                            | `entities/session` — [Session management](./session-management.md)                         |
...
| `files` of the override block (`steiger.config.ts`)               | `['./src/features/sign-in/**', './src/features/update-user-name/**']`                                                                                               | The slices for which `'fsd/insignificant-slice': 'off'` holds                                                                                                                                                                                                                                                                                 |
| `paths` (`tsconfig.app.json`)                                     | `"@/*": ["./src/*"]`                                                                                                                                                | The alias for `src/`, `env.d.ts` and `vitest.setup.ts`: `tsc -b`, ESLint's type-aware rules through `projectService`, and dependency-cruiser (`--ts-config tsconfig.app.json`, working tree only) resolve through it                                                                                                                          |
...
| `arch:graph` flags (`package.json`, working tree only)            | `--no-config`, `--ts-config tsconfig.app.json`, `--include-only '^src'`, `--exclude '\.test\.tsx?$'`, `--output-type mermaid`, `--collapse '^src/[^/]+/[^/]+'`      | No rule file, the app project's alias, `src` only, tests dropped, one node per slice or segment                                                                                                                                                                                                                                               |
...
7. **Run the architecture check.** `npm run arch` now fails:
   ...
   this repo's accepted exception, so add the slice to the override in `steiger.config.ts`:
...
8. **Run the remaining gates and refresh the graph.**
   ...
   [`docs/architecture-graph.md`](../architecture-graph.md); commit the graph with the slice. The
   `arch:graph` line only works in a working tree that already has the script — at `1c193c6` it
   exits with `npm error Missing script: "arch:graph"` (see the note under
...
The script and the file it writes are both working-tree additions — neither is in `1c193c6` — so
this section describes a command that has to land before a fresh clone can run it (see the note
...
- **The dependency graph is generated, never drawn.** `npm run arch:graph` derives the picture from
  ...
  and ESLint. Nothing checks that the generated file is current — and at `1c193c6` neither the
  script nor the file is committed at all; [Quality gates](./quality-gates.md) sketches a gate that
  would, once both land.
...
- Replace the whole array in `steiger.config.ts` with `[...fsd.configs.recommended]`:
  `npm run arch` reports `fsd/insignificant-slice` for `src/features/sign-in` and
  `src/features/update-user-name`.
...
- **`arch:graph`, `dependency-cruiser` and `docs/architecture-graph.md` are not in `1c193c6`.**
  This document's `Verified against` SHA is the repository's current `HEAD`, and `git status`
  still shows `package.json` and `package-lock.json` modified and `docs/` untracked. That
  commit's `package.json` lists no `arch:graph` script and no `dependency-cruiser`
  devDependency, and `git log --all -- docs/architecture-graph.md` returns nothing, so a reader
  who checks out the SHA cannot reproduce this document's graph material: `npm run arch:graph`
  fails with `npm error Missing script: "arch:graph"`, and the file every
  `../architecture-graph.md` link points at is absent from that tree. Everything else here is
  verified against `1c193c6`. The gap closes at the commit that lands `package.json`,
  `package-lock.json` and `docs/` together, and this bullet retires with it;
  [Quality gates](./quality-gates.md#known-limitations) records the same gap.
...
- **Nothing keeps the dependency graph current.** No gate regenerates or compares
  `docs/architecture-graph.md`, so once it is tracked it can drift from the imports between two
  runs of `npm run arch:graph` (see [Quality gates](./quality-gates.md#known-limitations)).
