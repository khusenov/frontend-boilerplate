...
that. (This is also why the `@/shared/testing` fence uses `import-x/no-restricted-paths`: a
different rule name cannot collide with the `no-restricted-imports` options at all.) Fourth, an `ignores` removes files from its own block only, so they take the
...
The rules are otherwise verified by running the gates — every push runs
