...
barrel, and regular-expression patterns. The file's path decides which lists apply: of the twelve
...
among its ten gates (see [Quality gates](./quality-gates.md)).
...
| `I18N_VENDOR_IMPORT_PATHS`, `FORM_VENDOR_IMPORT_PATHS`, `FORM_VENDOR_IMPORT_PATTERNS`, `TRANSPORT_VENDOR_IMPORT_PATHS`, `ERROR_BOUNDARY_VENDOR_IMPORT_PATHS`, `NOTIFICATIONS_VENDOR_IMPORT_PATHS` | outside layers · root config    | One fence per wrapped vendor                                                                                                                                                                             | `eslint.config.js`                                                           |
...
| `no-restricted-imports` blocks                                                                                                                                                                    | outside layers · root config    | Twelve blocks, in order, that apply the lists by path                                                                                                                                                    | `eslint.config.js`                                                           |
...
| `import-x/resolver-next`                                                                                                                                                                          | outside layers · root config    | Teaches `import-x` `.ts`/`.tsx` and the `@/` mapping; without it every path-based `import-x` rule skips `src/` in silence                                                                                | `eslint.config.js`                                                           |
| `import-x/no-restricted-paths`                                                                                                                                                                    | outside layers · root config    | Keeps the devDependency-backed `@/shared/testing` out of the production graph; test files are exempt                                                                                                     | `eslint.config.js`                                                           |
| `verify:import-fence`                                                                                                                                                                             | outside layers · root config    | Gate 10: lints two probe files to prove the fence fires for production paths and not for test paths                                                                                                      | `scripts/verify-import-fence.mjs`                                            |
...
| Segment barrels                                                                                                                                                                                   | `shared`                        | One public API per segment: `api`, `config`, `i18n`, `notifications`, `observability`, `testing`, `theme`                                                                                                | `src/shared/*/index.ts`                                                      |
...
| `shared`   | `src/shared`   | Business-free building blocks, as segments: `api`, `config`, `i18n`, `notifications`, `observability`, `testing`, `theme`, and the grouped `lib` (`cn`, `format-duration`, `single-flight`) and `ui` (`button`, `error-boundary`, `form`, `input`, `label`, beside the loose `theme.css`) | Nothing above `shared`; another segment only through its public API | Nine segments                                                       |
...
| `@/entities/user`              | `entities`   | `createUserMutations`, `createUserQueries`, `toUserId`, `UserStatusLabel`                                                                                                                                                                                                                                   | `User`, `UserId`, `UserNameChange`                                                                                                                                                                                                                                              | [User profile (read path)](./user-profile.md)                                                                                                                                              |
| `@/shared/api`                 | `shared`     | `createHttpClient`, `createQueryClient`, `HttpClientProvider`, `useHttpClient`, `HttpError`, `isHttpError`, `toHttpError`, `noContentSchema`, `parseResponse`                                                                                                                                               | `BearerTokenSource`, `CreateHttpClientOptions`, `ExchangeContext`, `HttpBodyRequestConfig`, `HttpClient`, `HttpQueryParams`, `HttpQueryParamValue`, `HttpRequestConfig`, `HttpRequestOptions`, `HttpErrorDetails`, `HttpErrorKind`, `ResponseValidationIssue`, `ResponseSchema` | [HTTP transport](./http-transport.md). `parseResponse` is importable only in `shared/api` and `shared/testing`                                                                             |
...
| `@/shared/notifications`       | `shared`     | `NotificationViewport`, `useNotifier`, `NotifierProvider`, `createSonnerNotifier`, `toSafeNotifier`                                                                                                                                                                                                         | `AppNotification`, `Notifier`                                                                                                                                                                                                                                                   | [Update user name (write path)](./update-user-name.md). Below `app`, only `AppNotification`, `Notifier`, `NotifierProvider` and `useNotifier` are importable                               |
...
| `@/shared/testing`             | `shared`     | `renderWithProviders`, `renderHookWithProviders`, `createHttpClientStub`, `parseStubResponse`, `createRecordingNotifier`, `createTestQueryClient`                                                                                                                                                           | `RecordingNotifier`, `RenderWithProvidersOptions`, `RenderWithProvidersResult`, `RenderHookWithProvidersOptions`, `RenderHookWithProvidersResult`                                                                                                                               | [Unit and component testing](./unit-testing.md). Importable only from `*.test.{ts,tsx}`; `import-x/no-restricted-paths` blocks every other file                                            |
...
session constructors, both `@/shared/observability` values and every `@/shared/notifications` name
outside the four the allow-list names — are banned in every layer below
...
| `fsd/ambiguous-slice-names`        | A slice named like a `shared` segment — here `api`, `config`, `i18n`, `lib`, `notifications`, `observability`, `testing`, `theme` or `ui`                                                                                                                                                                                                                                                                                            |
...
- The `src/app/{routes,router}/**` block writes its six **construction bans** and its `axios` fence
...
| `createBrowserThemeStorage`, `createDocumentThemeApplier`, `createSystemThemeSource`, `createThemeController` from `@/shared/theme`                                                                | Construction ban         | The same                                                                                         | None                                                            | `shared/theme` — [Design system](./design-system.md)                                                                            |
...
| `@/shared/notifications` except `AppNotification`, `Notifier`, `NotifierProvider`, `useNotifier` (`allowImportNames`)                                                                              | Construction ban         | The same                                                                                         | None                                                            | `shared/notifications` — [Update user name (write path)](./update-user-name.md)                                                 |
| `^@/shared/api/`, `^@/shared/observability/`, `^@/shared/theme/`, `^@/shared/notifications/`                                                                                                       | Deep-import patterns     | The same                                                                                         | None                                                            | [HTTP transport](./http-transport.md), [Error handling and reporting](./error-handling.md), [Design system](./design-system.md) |
...
| `sonner`                                                                                                                                                                                           | Vendor fence             | Every file in `src/`                                                                             | `src/shared/notifications/**`, tests included                   | `shared/notifications` — [Update user name (write path)](./update-user-name.md)                                                 |
...
| `parseResponse` from `@/shared/api` (`@typescript-eslint/no-restricted-imports`)                                                                                                                   | Parser fence             | Every file in `src/`                                                                             | `src/shared/api/**` and `src/shared/testing/**`, tests included | `shared/api` — [HTTP transport](./http-transport.md)                                                                            |
...
| --- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `src/**/*.{ts,tsx}`                                                          | `react-error-boundary`, `sonner`; the group patterns                                                                                                                                                                                                                                                                                          |
| 2   | `src/{entities,features,widgets,pages,shared}/**/*.{ts,tsx}`                 | `LOWER_LAYER_IMPORT_PATHS` (which carries the four theme constructors and the `@/shared/notifications` allow-list), `LOWER_LAYER_IMPORT_PATTERNS` (which carries `^@/shared/theme/` and `^@/shared/notifications/`) and every vendor list: the i18next packages, TanStack Form (path and patterns), `axios`, `react-error-boundary`, `sonner` |
...
| 7   | `src/shared/notifications/**/*.{ts,tsx}`                                     | Block 2's lists without `sonner`, and with the `@/shared/theme` construction ban widened to the whole barrel — the toast follows `.dark` through CSS, never through JavaScript                                                                                                                                                                |
| 8   | `src/shared/ui/form/**/*.{ts,tsx}`, ignoring its `*.test.{ts,tsx}`           | Block 2's lists without TanStack Form, plus the validator fence                                                                                                                                                                                                                                                                               |
| 9   | `src/shared/ui/error-boundary/**/*.{ts,tsx}`, ignoring its `*.test.{ts,tsx}` | Block 2's lists without `react-error-boundary`                                                                                                                                                                                                                                                                                                |
| 10  | `src/main.tsx`                                                               | `react-error-boundary`, `sonner`; the entry fence                                                                                                                                                                                                                                                                                             |
| 11  | `src/app/{routes,router}/**/*.{ts,tsx}`                                      | A list of its own: the six construction bans with route-specific messages, `axios`, the i18next packages, `@tanstack/react-form`, `react-error-boundary`, `sonner`; the group, `shared/api`, `shared/observability`, `shared/theme`, `shared/notifications` and slice public-API patterns                                                     |
| 12  | `e2e/**/*.ts`, `playwright.config.ts`                                        | The suite fence                                                                                                                                                                                                                                                                                                                               |
...
that. (This is also why the `@/shared/testing` fence uses `import-x/no-restricted-paths`, and the
parser fence `@typescript-eslint/no-restricted-imports`, in one self-contained block after all the
others: a different rule name cannot collide with the `no-restricted-imports` options at all.)
Fourth, an `ignores` removes files from its own block only, so they take the options of the last
other block that matches them: `src/shared/ui/form/form.test.tsx` matches blocks 1 and 2 and gets
block 2, where `@tanstack/react-form` values are banned and validators are not.
...
- **A new segment**, for a capability with its own port, is `src/shared/<segment>/` with an
  ...
  adds both for a hypothetical `shared/analytics`; `src/shared/notifications` is the shipped
...
2. Spread `...DATE_VENDOR_IMPORT_PATHS` into the `paths` of every block that fences a vendor today —
   eleven of the twelve, in file order: `src/**/*.{ts,tsx}`,
   ...
   `src/shared/notifications/**`, `src/shared/ui/form/**`, `src/shared/ui/error-boundary/**`,
   `src/main.tsx` and `src/app/{routes,router}/**`. A block left out is a hole: its files never see
   the fence.
...
Almost no test covers the boundary configuration itself. Vitest collects only
`src/**/*.{test,spec}.{ts,tsx}`, and `eslint.config.js` says of its order-sensitive blocks that
"nothing tests the flat config". The one exception is `scripts/verify-import-fence.mjs`, gate 10,
which lints a pair of throwaway probes to prove the `@/shared/testing` fence and the `import-x`
resolver it rests on are both live; block ordering for `no-restricted-imports` is still untested,
and so is the `parseResponse` fence, which was proven once by hand when it landed. The rules are
otherwise verified by running the gates — every push runs them through `npm run audit` — and by
tests that make one boundary executable:
...
- Add `import { parseResponse } from '@/shared/api';` to a module under `src/features`:
  `npm run lint` reports `@typescript-eslint/no-restricted-imports` with
  `parseResponse serves the transport and its test doubles.`
...
- **Deep imports into `shared/i18n` and `shared/config` from another `shared` segment pass both
  gates.** ESLint carries deep-import patterns for `shared/api`, `shared/notifications`,
  `shared/observability` and `shared/theme` only, and steiger skips same-layer imports.
...
- **Almost nothing tests the flat config.** A block appended below the carve-outs, or a vendor list
  left out of one block, silently disables a fence for those files; only a probe import reveals it.
  `verify:import-fence` automates exactly one such probe — the `@/shared/testing` fence and its
  resolver — and leaves every `no-restricted-imports` block unproven.
