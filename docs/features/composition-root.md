...
> **Status:** Complete · **Layers:** app, widgets, entities, shared, outside layers · **Verified against:** `d442a06`
...
1. **The host page.** `index.html` contains an empty `<div id="root"></div>` and one
   ...
   hashed entry chunk and adds a link to the entry stylesheet. The page also carries one inline,
   attribute-free `<script>` as the last child of `<head>`, and that script is the only application
   code that runs before the first paint: a module script is deferred, so anything React renders —
   including the theme — lands after the browser has already painted. It reads the stored preference,
   resolves it against `(prefers-color-scheme: dark)` and writes both the `dark` class and
   `style.colorScheme` onto `<html>`, then does nothing else. It is a hand-written mirror of
   `shared/theme`'s three adapters, held to byte-identical output by
   `src/app/entrypoint/theme-bootstrap.test.ts` (see
...
2. **Module evaluation binds the stateless half.** The `import { App } from '@/app'` at the top of
   ...
   `app-error-reporter.ts` to `toSafeErrorReporter(createConsoleErrorReporter())`, binds `notify` in
   `app-notifier.ts` to `toSafeNotifier(createSonnerNotifier())`, and builds two
...
4. **`App` renders the tree.** Everything below `App` is created by `App`, `AppProviders` and
   ...
   └─ App                                   src/app/entrypoint/app.tsx
      └─ ErrorBoundary                      AppCrashFallback, onError = handleRenderError
         └─ AppProviders                    builds transport, queryClient, i18n, theme
            └─ ThemeProvider                themeController, applyTheme
               ├─ DocumentThemeSync          applies the resolved theme in an effect
               └─ QueryClientProvider        queryClient
                  ├─ Suspense                  fallback = null
                  │  └─ I18nProvider           i18n
                  │     ├─ NotificationViewport  sonner's toast region, labels translated
                  ...
                  │                 └─ NotifierProvider   notify, passed in by app.tsx
...
5. **`AppProviders` builds the clients, once.** Its first render runs five `useState` lazy
   ...
   TanStack Query client whose failure callbacks feed the reporter; `createI18n()`, the i18next
   instance, which detects the locale synchronously ([Internationalization](./internationalization.md));
   `createThemeController({ storage: createBrowserThemeStorage(), systemTheme: createSystemThemeSource() })`,
   which reads the stored preference and opens no subscription until something listens; and
   `createDocumentThemeApplier()`, which closes over `document.documentElement`
   ([Design system](./design-system.md)). A lazy initializer runs on the first render only, so each
   instance keeps its identity for as long as `AppProviders` stays mounted. The providers publish
   them: `ThemeProvider` the theme controller, `QueryClientProvider` the query client, `I18nProvider`
   the i18n instance, `HttpClientProvider` `transport.httpClient`, and `SessionResolverProvider`,
   `SessionStarterProvider` and `SessionEnderProvider` the transport's `sessionResolver`,
   `sessionStarter` and `sessionEnder`. `NotifierProvider` publishes the one value `AppProviders`
   does not build — the `notifier` prop `app.tsx` passes in — and `NotificationViewport` mounts
   inside `I18nProvider`, where `useTranslation()` can reach an i18n instance.
...
importing `createHttpClient`, `createQueryClient`, `createI18n`, the four theme constructors
(`createThemeController`, `createBrowserThemeStorage`, `createSystemThemeSource`,
`createDocumentThemeApplier`), the six session constructors or any value from
`@/shared/observability`, so `app/entrypoint` is the only place a client can be built. `app/router` builds one concrete, the router, from ports it reads back out of React context
([Routing](./routing.md)).

| Component                                                                   | Layer                      | Responsibility                                                                                                                                                                                      | File                                                                                                                                                                           |
| --------------------------------------------------------------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Host page                                                                   | outside layers             | Holds the `#root` mount element, the module script that makes `src/main.tsx` Vite's entry, and the inline pre-paint theme script                                                                    | `index.html`                                                                                                                                                                   |
...
| `AppProviders`                                                              | app/entrypoint             | Owns every client in a `useState` initializer, publishes them through eight providers, mounts the `Suspense` boundary, `NotificationViewport` and `ReactQueryDevtools`, subscribes the cache policy | `src/app/entrypoint/app-providers.tsx`                                                                                                                                         |
...
| `notify`                                                                    | app/entrypoint             | The one binding of a concrete notifier, safe at construction; `app.tsx` prop-injects it into `AppProviders`                                                                                         | `src/app/entrypoint/app-notifier.ts`                                                                                                                                           |
...
| Theme controller, adapters and `ThemeProvider`                              | shared/theme               | Resolve the preference against the operating system; publish it and keep `<html class style>` in sync with the resolved theme ([Design system](./design-system.md))                                 | `src/shared/theme/theme-controller.ts`, `src/shared/theme/create-*.ts`, `src/shared/theme/theme-provider.tsx`                                                                  |
...
| `ThemeController` | `createThemeController({ storage, systemTheme })` over the browser storage and `matchMedia` adapters | `ThemeProvider`                                                                                                                                                                | `useTheme()` from `@/shared/theme`                                                                   | [Design system](./design-system.md)                                                          |
| `ThemeApplier`    | `createDocumentThemeApplier()`, closed over `document.documentElement`                               | A prop on `ThemeProvider`; its `DocumentThemeSync` child calls it in an effect                                                                                                 | Not readable below `app`                                                                             | [Design system](./design-system.md)                                                          |
...
| `Notifier`        | `notify`, `toSafeNotifier(createSonnerNotifier())` in `app-notifier.ts`                              | A prop on `AppProviders`, republished by `NotifierProvider`, which wraps it safe a second time                                                                                 | `useNotifier()` from `@/shared/notifications`                                                        | [Update user name (write path)](./update-user-name.md)                                       |
...
| Theme storage key (`create-browser-theme-storage.ts`, mirrored in `index.html`) | `'app.theme'`                                                      | Where the explicit preference is persisted, alongside i18next's `'app.locale'`. Two hand-maintained copies; `theme-bootstrap.test.ts` reads the adapter's own key and fails if the page's drifts from it                                                                                                                                                                                                                                               |
| Colour-scheme query (`create-system-theme-source.ts`, mirrored in `index.html`) | `'(prefers-color-scheme: dark)'`                                   | What "follow the system" listens to. Same two copies, same guard                                                                                                                                                                                                                                                                                                                                                                                       |
| Dark class name (`create-document-theme-applier.ts`, mirrored in `index.html`)  | `'dark'`                                                           | Owned by `src/shared/ui/theme.css` — these are mirrors, and the guard asserts the stylesheet still styles whatever the applier writes ([Design system](./design-system.md))                                                                                                                                                                                                                                                                            |
...
4. **Hand it to `AppProviders` and publish it.** `App` passes the tracker down as a prop, the way it
   ...
   inside another port's provider when its own component calls that port's hook. A provider that
   depends on nothing and renders no copy goes the other way, outside both, as `ThemeProvider` does:
   suspending it would be pointless and would let the theme arrive late, after a fallback. `Suspense`
   is for what needs translations; everything above it is what the translated tree may depend on.
...
5. **Test the wiring.** Add a case to `src/app/entrypoint/app-providers.test.tsx`, beside
   ...
   The prop is required, so every existing render in the file stops type-checking until each passes
   `analyticsTracker={vi.fn()}`. `src/shared/notifications` is the first worked instance of this
   whole walkthrough — read it alongside these five steps. The new segment's modules need co-located tests of their own —
...
- **The pre-paint script is a deliberate second implementation, and the only one.** Killing the
  flash of white needs code that runs before the first paint, and in a client-rendered Vite SPA
  nothing React renders qualifies: `<script type="module">` is deferred, so the earliest the tree
  can apply a theme is after the browser has already painted light. An external script would need a
  network round-trip first, which is worse. So the page carries an inline classic script, and it
  necessarily hardcodes the storage key, the class name, the media query, the preference vocabulary
  and the resolution rule — five things that can drift from `shared/theme` while every gate stays
  green. The mitigation is not discipline but a test: `theme-bootstrap.test.ts` executes the page's
...
- **The provider order follows from what reads what.** `AppRouterProvider`, the child, calls
  ...
  `SessionEnderProvider`.
  `ThemeProvider` is outermost because it depends on nothing and everything inside it may depend on
  it: its controller is built from two argument-free adapters, and putting it inside `Suspense`
  would let a suspended subtree delay the theme behind a fallback. It deliberately does not cover
  `AppCrashFallback` — the `ErrorBoundary` in `app.tsx` wraps `AppProviders` from outside, so a
  startup crash replaces this whole tree — and the fallback still renders themed anyway, because the
  theme is a class on `<html>` and pure CSS. `QueryClientProvider` is next because `ReactQueryDevtools`
  reads the same client from context and sits beside the application rather than inside it, outside
  `Suspense`, so a suspended subtree never takes the devtools with it. `Suspense` wraps `I18nProvider` and everything beneath
  ...
  `useSuspense: false` ([Internationalization](./internationalization.md)), and `DocumentThemeSync`
  reads a plain `useSyncExternalStore`. The six providers
  inside it — `I18nProvider`, `HttpClientProvider`, `SessionResolverProvider`,
  `SessionStarterProvider`, `SessionEnderProvider` and `NotifierProvider` — read nothing from one
  another's context, since every value comes from `AppProviders`' own state or its props, so their
  relative order is not load-bearing; what matters is that all of them enclose `children`. They nest
  the i18n instance first and then the transport's values — client, resolver, starter, ender — a
  convention rather than a constraint: `AuthenticatedTransport` lists `sessionEnder` second,
  alphabetically, while `NotifierProvider` is nested innermost.
  `NotificationViewport` is the one child in this stack whose position **is** load-bearing: it calls
  `useTranslation()` for the toast region's accessible name, and `createI18n` never calls
  `.use(initReactI18next)`, so outside `I18nProvider` there is no i18n instance in the browser and
  the region would ship as `aria-label="notifications.regionLabel alt+T"` in every locale. It fails
  silently — no suspend, no crash — and the unit tests cannot see it, because `vitest.setup.ts`
  installs a module-global instance; the anchored, case-sensitive `savedNotice()` locator in
  `e2e/page-objects/user-profile-page-object.ts` is the only gate that catches a hoisted viewport.
...
[Unit and component testing](./unit-testing.md)); the three without one are `app-error-reporter.ts`
and `app-notifier.ts`, each a single binding expression, and `app-router-context.ts`, an interface. One test has no module of
its own — `theme-bootstrap.test.ts` guards `index.html`, which sits outside the layer system and
cannot hold a test; `app/entrypoint` is the only place inside the system that participates in the
same capability, and the only one permitted to read the host page, the adapters and `theme.css` at
once:
...
| `src/app/entrypoint/app-providers.test.tsx`                                                                                    | Children render; the query client carries the configured `staleTime` of `30_000` and the HTTP client is usable; one HTTP client instance across re-renders; the cache clears when the session ends; the transport's starter, resolver and ender reach children; the injected notifier reaches children through `useNotifier()`, and the notification region carries its translated accessible name; a stored `dark` preference reaches `<html>` as both the class and `color-scheme`, and a stored `light` one leaves neither; a query failure reaches the injected `onQueryError` |
| `src/app/entrypoint/theme-bootstrap.test.ts`                                                                                   | The pre-paint mirror: the inline `<script>` in `index.html` is pulled out with `?raw`, run against key-aware stubs, and asserted to leave `<html>` byte-identical to what the real storage, system and applier adapters produce across all twelve stored-value × system-preference combinations; the script sits inside `<head>` before the module script; a throwing `localStorage` leaves the element untouched; and `theme.css` still styles whatever class the applier writes                                                                                                  |
