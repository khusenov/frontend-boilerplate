...
> **Status:** Complete · **Layers:** app, widgets, entities, shared, outside layers · **Verified against:** `5c55de1`
...
5. **`AppProviders` builds the clients, once.** Its first render runs five `useState` lazy
   ...
   `sessionStarter` and `sessionEnder`.
...
| `AppProviders`                                                              | app/entrypoint             | Owns every client in a `useState` initializer, publishes them through seven providers, mounts the `Suspense` boundary and `ReactQueryDevtools`, subscribes the cache policy | `src/app/entrypoint/app-providers.tsx`                                                                                                                                         |
...
- **The provider order follows from what reads what.** `AppRouterProvider`, the child, calls
  ...
  reads a plain `useSyncExternalStore`. The five providers
...
a single binding expression, and `app-router-context.ts`, an interface. One test has no module of
