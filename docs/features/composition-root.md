...
→ `widgets` → `features` → `entities` → `shared`; this repo has no `widgets` directory yet, so the
composition root binds nothing for that layer). `app` and `shared` are divided into purpose-named
**segments** (`app/entrypoint`, `app/router`, `app/routes`, `app/styles`), the layers between them
into **slices** (`entities/session`), and each slice or segment is consumed through its **public
API**, an `index.ts` barrel that only re-exports. A **provider** publishes one value to its React
subtree, and a hook such as `useHttpClient()` reads it back, throwing when no provider is above it.
...
([Error handling and reporting](./error-handling.md)), which reaches the root boundary as `onError`
because that boundary sits above every provider. The root's own types are narrow:
...
One line per binding; the owner documents the seam in depth.
...
- **Some wiring is unasserted.** Identity across re-renders is tested for the HTTP client and the
  ...
  no gate checks that devtools stay out of the production bundle.
