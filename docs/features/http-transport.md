...
- the **unauthenticated client** — `sendCookies: true`, no token source — which it hands to
  `createSessionApi` for `POST /auth/refresh` and `POST /auth/login`, and which never leaves the
  factory;
...
| `createAuthenticatedTransport`                                                           | `app/entrypoint`                    | Builds the unauthenticated and the authenticated client and binds `createSessionTokenSource` to the latter                                                                                         | `src/app/entrypoint/create-authenticated-transport.ts`                             |
...
| `createSessionApi`                                                                       | `entities/session · api`            | `POST /auth/refresh` and `POST /auth/login` on the unauthenticated client; turns `HttpError` statuses into outcomes                                                                                | `src/entities/session/api/session-api.ts`                                          |
...
- **`noContentSchema` lives beside the port.** It is ten lines that keep `shared/api` free of a
  ...
  a second schema owned by `shared/api` is the trigger to split the module.
...
| `src/app/entrypoint/create-authenticated-transport.test.ts` | The two-client composition end to end: renew and replay through a real refresh call, no recursion when the refresh itself answers 401, and the first request after a sign-in or a resolved session carrying the bearer token ([Session management](./session-management.md))                                                                                                                                                                                                                                                                                                    |
